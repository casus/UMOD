...
from .customNumpyArrayIterator import customNumpyArrayIterator
from .utils import (apply_random_patch_gen, array_to_img, flip_axis,
...
class customImageDataGenerator:
    ...
    def __init__(
        ...
        max_iter = 10,
        thresh_obj_perc = None,
        target_width = None,
        target_height = None,
        image_aug_check = None,
        mask_aug_check = None,
        dtype=None,
    ):
        ...
        if thresh_obj_perc is None or not 0 < thresh_obj_perc < 1:
            raise ValueError(
                "`thresh_obj_perc` cannot be None and must be strictly between 0 and 1. "
                " Received: %s" % thresh_obj_perc
            )
        self.thresh_obj_perc = thresh_obj_perc

        if image_aug_check is not None and not isinstance(image_aug_check, dict):
            raise ValueError(
                "`image_aug_check` must be a dict with customImageDataGenerator arguments as keys and boolean values. "
            )
        elif image_aug_check is not None and isinstance(image_aug_check, dict):
            if mask_aug_check is None:
                raise ValueError(
                "`mask_aug_check` cannot be None if `image_aug_check` is not None and should be in same format as `image_aug_check`. "
            )
            elif mask_aug_check is not None and not isinstance(mask_aug_check, dict):
                raise ValueError(
                "`image_aug_check` must be a dict with customImageDataGenerator arguments as keys and boolean values. "
            )
            elif mask_aug_check is not None and isinstance(mask_aug_check, dict):
                self.image_aug_check = image_aug_check
                self.mask_aug_check = mask_aug_check
    ...
    def standardize(self, x, aug_check):
        ...
        """
        if self.preprocessing_function and aug_check.get("preprocessing_function", False):
            x = self.preprocessing_function(x)
        if self.rescale and aug_check.get("rescale", False) :
            x *= self.rescale
        if self.samplewise_center and aug_check.get("samplewise_center", False) :
            x -= np.mean(x, keepdims=True)
        if self.samplewise_std_normalization and aug_check.get("samplewise_std_normalization", False) :
            x /= np.std(x, keepdims=True) + 1e-6

        if self.featurewise_center and aug_check.get("featurewise_center", False) :
        ...
        if self.featurewise_std_normalization and aug_check.get("featurewise_std_normalization", False) :
        ...
        if self.zca_whitening and aug_check.get("zca_whitening", False) :
    ...
    def get_random_transform(self, img_shape, seed=None):
        ...
        if self.target_height is not None and self.target_width is not None :
            ...
            rand_x = np.random.randint(img_shape[0]-target_width)
            rand_y = np.random.randint(img_shape[1]-target_height)
    ...
    def apply_transform(self, x, transform_parameters,aug_check):
        ...
        if 'fill_mode' in aug_check and aug_check.get('fill_mode'):
            fill_mode = self.fill_mode
        else:
            fill_mode = "nearest"

        if 'cval' in aug_check and aug_check.get('cval'):
            cval = self.cval
        else:
            cval = 0.0

        if 'interpolation_order' in aug_check and aug_check.get('interpolation_order'):
            interpolation_order = self.interpolation_order
        else:
            interpolation_order = 1

        if aug_check.get("rotation_range",False):
            theta = transform_parameters.get("theta", 0)
        else:
            theta = 0

        if aug_check.get("height_shift_range",False):
            tx = transform_parameters.get("tx", 0)
        else:
            tx = 0

        if aug_check.get("width_shift_range",False):
            ty = transform_parameters.get("ty", 0)
        else:
            ty = 0

        if aug_check.get("shear_range",False):
            shear = transform_parameters.get("shear", 0)
        else:
            shear = 0

        if aug_check.get("zoom_range", False):
        ...
        if transform_parameters.get("channel_shift_intensity") is not None and aug_check.get('channel_shift_range', False):
        ...
        if transform_parameters.get("flip_horizontal", False) == True and aug_check.get("horizontal_flip", False):
            x = flip_axis(x, img_col_axis)

        if transform_parameters.get("flip_vertical", False) == True and aug_check.get("vertical_flip", False):
            x = flip_axis(x, img_row_axis)

        if transform_parameters.get("brightness") is not None and aug_check.get("brightness_range", False):
            x = apply_brightness_shift(x,
                                        transform_parameters.get("brightness"),
                                        False)

        if transform_parameters.get("target_width") is not None and aug_check.get("target_width", False):
            x = apply_random_patch_gen(x,
                                        transform_parameters.get("rand_x"),
                                        transform_parameters.get("rand_y"),
                                        transform_parameters.get("target_width"),
                                        transform_parameters.get("target_height"),
                                        )
        return x

    def random_transform(self, x, aug_check,seed=None):
        ...
        return self.apply_transform(x,params, aug_check)

    def fit(self, x, aug_check,augment=False, rounds=1, seed=None):
        ...
        if augment:
            ax = np.zeros(
                tuple([rounds * x.shape[0]] + list((self.target_height,self.target_width,1))),
                dtype=self.dtype,
            )
            ...
            for r in range(rounds):
                for i in range(x.shape[0]):
                    ax[i + r * x.shape[0]] = self.random_transform(x[i], self.image_aug_check,seed)
            x = ax
